...
async function generarResultadosRandom() {
  try {
    ...
    for (const partido of partidosPendientes) {
      ...
      const hubo_roja = Math.random() < 0.2; // 20%
      const hubo_penal = Math.random() < 0.3; // 30%
      ...
      for (let pred of predicciones) {
        ...
        // --- Puntos por Eventos Especiales ---
        if (hubo_roja === false && pred.prediccion_roja === false) puntosSumados += 1;
        else if (hubo_roja === true && pred.prediccion_roja === true) puntosSumados += 2;

        if (hubo_penal === false && pred.prediccion_penal === false) puntosSumados += 1;
        else if (hubo_penal === true && pred.prediccion_penal === true) puntosSumados += 2;
